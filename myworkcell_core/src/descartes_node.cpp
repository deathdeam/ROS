...
//used to find the joint state which is a double vector.
//Pose or state of the joint is stored in the sensor_msgs
...
//line or trajectory is simply a vector of points
EigenSTL::vector_Isometry3d makeLine(const Eigen::Vector3d& start, const Eigen::Vector3d& stop, double ds)
{
  ...
  //step = distance / (distance per step)
...
public:
                ...
                myworkcell_core::PlanCartesianPathResponse& res)
  {
    ...
    //the planPath here is descartes_planner::DensePlanner::planPath
                           ...
                           const EigenSTL::vector_Isometry3d& path)
  {
    ...
    //transform msgs to eigen which could be used in the future.
    tf::poseMsgToEigen(reference, ref);

    for (auto& point : path)
    {
      // TODO: make a Descartes "cartesian" point with some kind of constraints
      descartes_core::TrajectoryPtPtr pt = makeTolerancedCartesianPoint( point*ref );
      //What does makeTolerancedCartesianPoint mean?
  ...
  descartes_core::TrajectoryPtPtr makeTolerancedCartesianPoint(const Eigen::Isometry3d& pose)
  {
    ...
    return TrajectoryPtPtr( new AxialSymmetricPt(pose,90,AxialSymmetricPt::FreeAxis::Z_AXIS)/* TODO: Specific point constructor here */ );
  }
